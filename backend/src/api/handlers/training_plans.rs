...
use crate::application::training_plans::{
    ...
    save_training_plan as save_training_plan_service, TrainingPlanServiceError,
};
use crate::domain::{
...
fn map_training_plan_service_error(error: TrainingPlanServiceError) -> ApiError {
    match error {
        ...
        TrainingPlanServiceError::Validation(message) => ApiError::Validation(message),
...
pub(crate) async fn create_training_plan(
    ...
    Extension(session): Extension<AuthenticatedSession>,
    Json(request): Json<TrainingPlanDefinitionRequest>,
) -> Result<(axum::http::StatusCode, Json<TrainingPlanSaveResponse>), ApiError> {
    let result =
        create_training_plan_service(&state.repository, &session.user_id, request.into_domain())
            .await
            .map_err(map_training_plan_service_error)?;
    Ok((
        axum::http::StatusCode::CREATED,
        Json(TrainingPlanSaveResponse {
...
pub(crate) async fn save_training_plan(
    ...
    Path(training_plan_id): Path<String>,
    Json(request): Json<TrainingPlanDefinitionRequest>,
) -> Result<Json<TrainingPlanSaveResponse>, ApiError> {
    let result = save_training_plan_service(
        &state.repository,
        &training_plan_id,
        &session.user_id,
        request.into_domain(),
    ...
    .await
    .map_err(map_training_plan_service_error)?;
    Ok(Json(TrainingPlanSaveResponse {
        training_plan_id: result.training_plan_id,
        version_number: result.version_number,
        created_new_version: result.created_new_version,
    }))
