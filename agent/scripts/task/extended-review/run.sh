...
COMMON_REVIEW_VALIDATION_GUIDANCE="Validate the findings artifact before finalize with: python3 agent/scripts/check/validate-artifact.py extended-review-findings agent/tmp/${TASK_NAME}-findings.yaml. Keep scalar text fields as YAML strings, not lists/objects."
...
echo "VALIDATE_WRITE_COMMAND=python3 agent/scripts/check/validate-artifact.py extended-review-findings agent/tmp/${TASK_NAME}-findings.yaml"
