...
class PbConfiguratorGymEditorScreenElement extends HTMLElement {
  ...
  #onTextInput = (field: string, value: string): void => {
    ...
    if (error && fieldElement) { const message = document.createElement("span"); message.className = "configurator-field-error"; message.id = "configurator-gym-name-error"; message.textContent = error; fieldElement.append(message); }
  ...
  #render(): void {
    ...
    const form = this.#state.isLoading ? '<p class="start-status" role="status">Loading gym detail...</p>' : this.#state.errorMessage ? `<p class="start-error" role="alert">${escapeHtml(this.#state.errorMessage)}</p>` : !isCreate && !detail ? '<p class="start-error" role="alert">Unable to find that Gym right now.</p>' : `
      <div class="configurator-editor-card"><label class="configurator-field" for="configurator-gym-name"><span class="configurator-field-label">Name</span><input id="configurator-gym-name" class="configurator-control" data-field="name" value="${escapeHtml(this.#nameDraft)}" required aria-required="true" aria-invalid="${error ? "true" : "false"}"${error && this.#touched ? ' aria-describedby="configurator-gym-name-error"' : ""} ${this.#isSaving || this.#isDeleting ? "disabled" : ""} />${error && this.#touched ? `<span id="configurator-gym-name-error" class="configurator-field-error">${escapeHtml(error)}</span>` : ""}</label>
      ${detail ? `<dl class="configurator-metadata"><div><dt>Status</dt><dd>${formatConfiguratorLifecycleStatus(detail.status)}</dd></div><div><dt>Stations</dt><dd>${(this.#state.stations ?? []).length === 1 ? "1 station" : `${(this.#state.stations ?? []).length} stations`}</dd></div></dl>` : ""}
      ${this.#submitError ? `<p class="start-error" role="alert">${escapeHtml(this.#submitError)}</p>` : ""}
      <div class="configurator-editor-actions"><button type="button" class="configurator-action-primary" data-ui-action="save-gym" ${disabled ? "disabled" : ""}>${this.#isSaving ? "Saving..." : isCreate ? "Create Gym" : "Save Name"}</button>${canDelete ? `<button type="button" class="configurator-action-danger" data-ui-action="delete-gym" ${this.#isDeleting ? "disabled" : ""}>${this.#isDeleting ? "Deleting..." : "Delete Draft"}</button>` : ""}</div></div>`;
    const stations = this.#state.stations ?? [];
    const stationSection = !isCreate && detail ? `<section class="configurator-gym-stations" aria-label="Stations"><div class="configurator-gym-stations-header"><h2>Stations</h2><button type="button" class="configurator-action-nested-add action-button" data-ui-action="start-configurator-station-create">+ New Station</button></div>${stations.length === 0 ? '<p class="start-copy">No stations yet.</p>' : `<ul class="configurator-gym-station-list">${stations.map((station) => `<li><button type="button" class="configurator-gym-station-row" data-ui-action="open-configurator-station-detail" data-station-id="${escapeHtml(station.id)}" aria-label="Open ${escapeHtml(station.name)} station"><span><strong>${escapeHtml(station.name)}</strong><small>${escapeHtml(station.load_profile.name)}</small></span><pb-configurator-status value="${escapeHtml(station.status)}"></pb-configurator-status></button></li>`).join("")}</ul>`}</section>` : "";
    const warning = this.#renameWarningOpen ? `<div class="confirm-dialog-layer" role="presentation"><div class="confirm-dialog-backdrop" role="presentation"></div><section class="confirm-dialog" role="alertdialog" aria-modal="true" aria-label="Historical rename warning"><p class="confirm-dialog-message">Renaming an active or inactive Gym can affect how historical workouts are understood. Save this name change?</p><div class="confirm-dialog-actions"><button type="button" class="configurator-action-dismiss" data-ui-action="dismiss-historical-rename-warning">Keep Editing</button><button type="button" class="configurator-action-primary" data-ui-action="save-gym">Save Name</button></div></section></div>` : this.#deleteWarningOpen ? `<div class="confirm-dialog-layer" role="presentation"><div class="confirm-dialog-backdrop" role="presentation"></div><section class="confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="delete-gym-warning-title"><h2 class="confirm-dialog-title" id="delete-gym-warning-title">Delete draft gym?</h2><p class="confirm-dialog-message">This will also delete ${stations.length} ${stations.length === 1 ? "station" : "stations"}.</p><div class="confirm-dialog-actions"><button type="button" class="configurator-action-dismiss" data-ui-action="dismiss-delete-gym-warning">Cancel</button><button type="button" class="configurator-action-danger" data-ui-action="delete-gym">Delete</button></div></section></div>` : "";
    this.innerHTML = `<div class="app-screen-shell"><button type="button" class="side-menu-toggle detail-back-button" data-ui-action="navigate-back-from-configurator-gym-detail" aria-label="Back"><span aria-hidden="true">←</span></button><section class="screen-panel configurator-gym-editor-screen" aria-label="Gym editor"><header class="exercise-variant-detail-header configurator-app-header"><img class="start-banner" src="/images/banner.png?v=20260401-2" alt="PumpBuddy banner" /><h1 class="exercise-variant-detail-header-title">Gym</h1></header>${form}${stationSection}</section>${warning}</div>`;
  }
}
