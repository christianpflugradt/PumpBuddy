...
class PbConfiguratorGymEditorScreenElement extends HTMLElement {
  ...
  #render(): void {
    ...
    if (dialog) dialog.state = this.#renameWarningOpen ? { accessibleName: "Historical rename warning", message: "Renaming an active or inactive Gym can affect how historical workouts are understood. Save this name change?", dismissAction: "dismiss-historical-rename-warning", dismissLabel: "Keep Editing", confirmAction: "save-gym", confirmLabel: "Save Name" } : { accessibleName: "Delete draft gym?", title: "Delete draft gym?", message: `This will also delete ${stations.length} ${stations.length === 1 ? "station" : "stations"}.`, dismissAction: "dismiss-delete-gym-warning", dismissLabel: "Cancel", confirmAction: "delete-gym", confirmLabel: "Delete", intent: "destructive" };
  }
}
