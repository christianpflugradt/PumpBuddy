...
class PbConfiguratorTrainingPlansScreenElement extends HTMLElement {
  ...
  #renderList(): string {
    ...
    return `<div class="configurator-entity-list" aria-label="Training plans">${this.#state.trainingPlans.map((plan) => `<button type="button" class="configurator-entity-row" data-ui-action="open-configurator-training-plan-detail" data-training-plan-id="${escapeHtml(plan.id)}" aria-label="Open ${escapeHtml(plan.name)} training plan"><span class="configurator-entity-row-name">${escapeHtml(plan.name)}</span><span class="configurator-entity-row-metadata configurator-training-plan-card-exercise-count">${plan.exercise_count === 1 ? "1 exercise" : `${plan.exercise_count} exercises`}</span></button>`).join("")}</div>`;
  }
  #renderCreate(): string {
    ...
    const exercisePicker = this.#exercisePickerOpen ? `<div class="secs-picker-layer" role="presentation"><button type="button" class="secs-picker-backdrop" data-ui-action="dismiss-create-plan-exercise-picker" aria-label="Close Exercise picker"></button><section class="secs-picker-sheet configurator-load-profile-picker" role="dialog" aria-modal="true" aria-labelledby="create-plan-exercise-picker-title"><header class="configurator-load-profile-picker-header"><h2 id="create-plan-exercise-picker-title" class="secs-picker-title">Choose Exercise</h2><input class="configurator-control" type="search" data-role="exercise-search" value="${escapeHtml(this.#exerciseQuery)}" placeholder="Search Exercises" aria-label="Search Exercises" autocomplete="off" /></header><div class="configurator-load-profile-picker-options" role="listbox" aria-label="Available Exercises">${availableExercises.length === 0 ? '<p class="start-copy">No Exercises available.</p>' : availableExercises.map((item) => `<button type="button" class="configurator-load-profile-picker-option" data-ui-action="select-create-plan-exercise" data-exercise-id="${escapeHtml(item.id)}" role="option"><span>${escapeHtml(item.name)}</span></button>`).join("")}</div></section></div>` : "";
    const variantPicker = this.#variantPickerOpen ? `<div class="secs-picker-layer" role="presentation"><button type="button" class="secs-picker-backdrop" data-ui-action="dismiss-create-plan-variant-picker" aria-label="Close Variant picker"></button><section class="secs-picker-sheet configurator-load-profile-picker" role="dialog" aria-modal="true" aria-labelledby="create-plan-variant-picker-title"><header class="configurator-load-profile-picker-header"><h2 id="create-plan-variant-picker-title" class="secs-picker-title">Choose Variant</h2><input class="configurator-control" type="search" data-role="variant-search" value="${escapeHtml(this.#variantQuery)}" placeholder="Search Variants" aria-label="Search Variants" autocomplete="off" /></header><div class="configurator-load-profile-picker-options" role="listbox" aria-label="Available Variants">${availableVariants.length === 0 ? '<p class="start-copy">No Variants available to add.</p>' : availableVariants.map((variant) => `<button type="button" class="configurator-load-profile-picker-option" data-ui-action="select-create-plan-variant" data-variant-id="${escapeHtml(variant.id)}" role="option"><span>${escapeHtml(variant.name)}</span></button>`).join("")}</div></section></div>` : "";
    const nameInvalid = this.#submitError === "Plan name is required.";
    const exerciseInvalid = this.#submitError === "Choose an initial exercise.";
    const variantsInvalid = this.#submitError === "Choose at least one allowed variant.";
    const errorReference = this.#submitError ? ' aria-describedby="configurator-training-plan-create-error"' : "";
    return `<button type="button" class="side-menu-toggle detail-back-button" data-ui-action="navigate-back-from-configurator-training-plan-create" aria-label="Back to Training Plans"><span aria-hidden="true">←</span></button><form class="configurator-training-plan-editor-card" aria-label="New training plan" onsubmit="return false"><label class="configurator-field" for="configurator-training-plan-create-name"><span class="configurator-field-label">Plan name</span><input id="configurator-training-plan-create-name" class="configurator-control" data-role="plan-name" value="${escapeHtml(this.#name)}" autocomplete="off" required aria-required="true" aria-invalid="${nameInvalid}"${nameInvalid ? errorReference : ""} /></label><section class="configurator-training-plan-exercise-card" aria-label="Initial exercise"><p id="configurator-training-plan-create-exercise-label" class="configurator-training-plan-variants-heading">Initial Exercise</p>${exercise ? `<p class="start-copy">${escapeHtml(exercise.name)}</p>` : '<p class="start-copy">Choose the first Exercise for this Training Plan.</p>'}<button id="configurator-training-plan-create-exercise" type="button" class="configurator-action-nested-add" data-ui-action="open-create-plan-exercise-picker" aria-labelledby="configurator-training-plan-create-exercise-label" aria-required="true" aria-invalid="${exerciseInvalid}"${exerciseInvalid ? errorReference : ""} aria-haspopup="dialog" aria-expanded="${this.#exercisePickerOpen}">${exercise ? "Change Exercise" : "Choose Exercise"}</button></section>${exercise ? `<section class="configurator-training-plan-exercise-card" aria-label="Allowed variants"><p id="configurator-training-plan-create-variants-label" class="configurator-training-plan-variants-heading">Allowed Variants</p>${selectedVariants.length ? `<ul class="configurator-training-plan-variant-list">${selectedVariants.map((variant) => `<li class="configurator-training-plan-variant"><strong>${escapeHtml(variant.name)}</strong><button type="button" class="configurator-training-plan-remove" data-ui-action="remove-create-plan-variant" data-variant-id="${escapeHtml(variant.id)}" aria-label="Remove ${escapeHtml(variant.name)}"><span aria-hidden="true">×</span></button></li>`).join("")}</ul>` : '<p class="start-copy">Choose at least one allowed Variant.</p>'}${exercise.variants.length > selectedVariants.length ? `<button id="configurator-training-plan-create-variants" type="button" class="configurator-action-nested-add" data-ui-action="open-create-plan-variant-picker" aria-labelledby="configurator-training-plan-create-variants-label" aria-required="true" aria-invalid="${variantsInvalid}"${variantsInvalid ? errorReference : ""} aria-haspopup="dialog" aria-expanded="${this.#variantPickerOpen}">+ Add Variant</button>` : ""}${exercise.variants.length === 0 ? '<p class="start-copy">This Exercise has no available Variants yet.</p>' : ""}</section>` : ""}${this.#submitError ? `<p id="configurator-training-plan-create-error" class="start-error" role="alert">${escapeHtml(this.#submitError)}</p>` : ""}<button type="button" class="configurator-action-primary" data-ui-action="save-configurator-training-plan"${this.#isSaving ? " disabled" : ""}>${this.#isSaving ? "Creating..." : "Create Training Plan"}</button></form>${exercisePicker}${variantPicker}`;
  }
  #render(): void { const isList = this.#state.mode === "list"; this.innerHTML = `<div class="app-screen-shell"><pb-side-menu mode="configurator" active-screen="configurator-training-plans" menu-id="configurator-training-plans-side-menu"></pb-side-menu><section class="screen-panel configurator-training-plans-screen" aria-label="Configurator training plans screen"><pb-configurator-header title="${isList ? "Training Plans" : "New Training Plan"}" context="${isList ? "Manage the current definitions for your workouts." : "Choose an initial Exercise and one or more allowed Variants."}" banner></pb-configurator-header>${isList ? '<pb-create-button action="start-configurator-training-plan-create" label="New Training Plan"></pb-create-button>' + this.#renderList() : this.#renderCreate()}</section></div>`; }
}
