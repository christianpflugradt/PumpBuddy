...
class PbConfiguratorTrainingPlansScreenElement extends HTMLElement {
  ...
  #renderCreate(): string {
    ...
    return `<button type="button" class="configurator-gym-back-button" data-ui-action="navigate-back-from-configurator-training-plan-create">‹ Back to Training Plans</button><form class="configurator-placeholder-card" aria-label="New training plan" onsubmit="return false"><label class="start-label">Plan name<input data-role="plan-name" value="${escapeHtml(this.#name)}" autocomplete="off" /></label><label class="start-label">Initial exercise<select data-role="plan-exercise"><option value="">Choose an exercise</option>${this.#state.exercises.map((item) => `<option value="${escapeHtml(item.id)}"${item.id === this.#exerciseId ? " selected" : ""}>${escapeHtml(item.name)}</option>`).join("")}</select></label>${exercise ? `<fieldset><legend>Allowed variants</legend>${variants.length ? variants.map((variant) => `<label><input type="checkbox" data-role="plan-variant" value="${escapeHtml(variant.id)}"${this.#variantIds.has(variant.id) ? " checked" : ""} /> ${escapeHtml(variant.name)}</label>`).join("") : '<p class="start-copy">This Exercise has no available Variants yet.</p>'}</fieldset>` : ""}${this.#submitError ? `<p class="start-error" role="alert">${escapeHtml(this.#submitError)}</p>` : ""}<button type="button" class="nav-button nav-button-primary action-button action-button-primary" data-ui-action="save-configurator-training-plan"${this.#isSaving ? " disabled" : ""}>${this.#isSaving ? "Creating..." : "Create Training Plan"}</button></form>`;
  }
  #render(): void { const isList = this.#state.mode === "list"; this.innerHTML = `<div class="app-screen-shell"><pb-side-menu mode="configurator" active-screen="configurator-training-plans" menu-id="configurator-training-plans-side-menu"></pb-side-menu><section class="screen-panel configurator-gyms-screen" aria-label="Configurator training plans screen"><header class="app-header configurator-app-header"><img class="start-banner" src="/images/banner.png?v=20260401-2" alt="PumpBuddy banner" /><h1 class="app-title">${isList ? "Training Plans" : "New Training Plan"}</h1><p class="start-copy">${isList ? "Manage the current definitions for your workouts." : "Choose an initial Exercise and one or more allowed Variants."}</p></header>${isList ? '<pb-create-button action="start-configurator-training-plan-create" label="New Training Plan"></pb-create-button>' + this.#renderList() : this.#renderCreate()}</section></div>`; }
}
