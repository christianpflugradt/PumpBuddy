...
class PbConfiguratorTrainingPlansScreenElement extends HTMLElement {
  ...
  #renderCreate(): string {
    ...
    return `<button type="button" class="side-menu-toggle detail-back-button" data-ui-action="navigate-back-from-configurator-training-plan-create" aria-label="Back to Training Plans"><span aria-hidden="true">←</span></button><form class="configurator-training-plan-editor-card" aria-label="New training plan" onsubmit="return false"><label class="configurator-field"><span class="configurator-field-label">Plan name</span><input class="configurator-control" data-role="plan-name" value="${escapeHtml(this.#name)}" autocomplete="off" /></label><section class="configurator-training-plan-exercise-card" aria-label="Initial exercise"><p class="configurator-training-plan-variants-heading">Initial Exercise</p>${exercise ? `<p class="start-copy">${escapeHtml(exercise.name)}</p>` : '<p class="start-copy">Choose the first Exercise for this Training Plan.</p>'}<button type="button" class="configurator-action-nested-add" data-ui-action="open-create-plan-exercise-picker" aria-haspopup="dialog" aria-expanded="${this.#exercisePickerOpen}">${exercise ? "Change Exercise" : "Choose Exercise"}</button></section>${exercise ? `<section class="configurator-training-plan-exercise-card" aria-label="Allowed variants"><p class="configurator-training-plan-variants-heading">Allowed Variants</p>${selectedVariants.length ? `<ul class="configurator-training-plan-variant-list">${selectedVariants.map((variant) => `<li class="configurator-training-plan-variant"><strong>${escapeHtml(variant.name)}</strong><button type="button" class="configurator-training-plan-remove" data-ui-action="remove-create-plan-variant" data-variant-id="${escapeHtml(variant.id)}" aria-label="Remove ${escapeHtml(variant.name)}"><span aria-hidden="true">×</span></button></li>`).join("")}</ul>` : '<p class="start-copy">Choose at least one allowed Variant.</p>'}${exercise.variants.length > selectedVariants.length ? `<button type="button" class="configurator-action-nested-add" data-ui-action="open-create-plan-variant-picker" aria-haspopup="dialog" aria-expanded="${this.#variantPickerOpen}">+ Add Variant</button>` : ""}${exercise.variants.length === 0 ? '<p class="start-copy">This Exercise has no available Variants yet.</p>' : ""}</section>` : ""}${this.#submitError ? `<p class="start-error" role="alert">${escapeHtml(this.#submitError)}</p>` : ""}<button type="button" class="configurator-action-primary" data-ui-action="save-configurator-training-plan"${this.#isSaving ? " disabled" : ""}>${this.#isSaving ? "Creating..." : "Create Training Plan"}</button></form>${exercisePicker}${variantPicker}`;
  }
