...
class PbConfiguratorStationEditorScreenElement extends HTMLElement {
  ...
  #renderCompatibilitySummary(): string {
    ...
    const edit = `<button type="button" class="configurator-action-secondary configurator-station-compatibility-edit" data-ui-action="open-configurator-station-compatibility-picker">Edit</button>`;
  ...
  #render(): void {
    ...
    const metadata = station && historical ? `<dl class="configurator-metadata"><div><dt>Gym</dt><dd>${escapeHtml(this.#state.gymName ?? this.#state.gymId)}</dd></div><div><dt>Load Profile</dt><dd>${escapeHtml(station.load_profile.name)}</dd></div><div><dt>Status</dt><dd>${formatConfiguratorLifecycleStatus(station.status)}</dd></div></dl>` : "";
    const selectedProfile = profiles.find((profile) => profile.id === this.#loadProfileIdDraft);
    const profileField = historical ? "" : `<div class="configurator-field"><span id="configurator-station-load-profile-label" class="configurator-field-label">Load Profile</span><button id="configurator-station-load-profile" type="button" class="configurator-control configurator-load-profile-picker-trigger" data-ui-action="open-load-profile-picker" aria-labelledby="configurator-station-load-profile-label" aria-required="true" aria-invalid="${loadProfileError ? "true" : "false"}"${loadProfileError && this.#touched ? ' aria-describedby="configurator-station-load-profile-error"' : ""} aria-haspopup="dialog" aria-expanded="${this.#loadProfilePickerOpen ? "true" : "false"}" ${this.#isSaving || this.#isDeleting ? "disabled" : ""}>${escapeHtml(selectedProfile?.name ?? "Choose a Load Profile")}</button>${loadProfileError && this.#touched ? `<span id="configurator-station-load-profile-error" class="configurator-field-error">${escapeHtml(loadProfileError)}</span>` : ""}</div>`;
    const warning = this.#renameWarningOpen ? "<pb-confirm-dialog></pb-confirm-dialog>" : "";
    this.innerHTML = `<div class="app-screen-shell"><button type="button" class="side-menu-toggle detail-back-button" data-ui-action="navigate-back-from-configurator-station-detail" aria-label="Back"><span aria-hidden="true">←</span></button><section class="screen-panel configurator-gym-editor-screen" aria-label="Station editor"><pb-configurator-header title="${station ? "Station" : "New Station"}" banner></pb-configurator-header><div class="configurator-editor-card"><label class="configurator-field" for="configurator-station-name"><span class="configurator-field-label">Name</span><input id="configurator-station-name" class="configurator-control" data-field="name" value="${escapeHtml(this.#nameDraft)}" required aria-required="true" aria-invalid="${nameError ? "true" : "false"}"${nameError && this.#touched ? ' aria-describedby="configurator-station-name-error"' : ""} ${this.#isSaving || this.#isDeleting ? "disabled" : ""} />${nameError && this.#touched ? `<span id="configurator-station-name-error" class="configurator-field-error">${escapeHtml(nameError)}</span>` : ""}</label>${profileField}${metadata}${this.#renderCompatibilitySummary()}${this.#submitError ? `<p class="start-error" role="alert">${escapeHtml(this.#submitError)}</p>` : ""}<div class="configurator-editor-actions"><button type="button" class="configurator-action-primary" data-ui-action="save-configurator-station" ${disabled ? "disabled" : ""}>${this.#isSaving ? "Saving..." : station ? historical ? "Save Name" : "Save Changes" : "Create Station"}</button>${station?.status === "new" ? `<button type="button" class="configurator-action-danger" data-ui-action="delete-configurator-station" ${this.#isDeleting ? "disabled" : ""}>${this.#isDeleting ? "Deleting..." : "Delete Draft"}</button>` : ""}</div></div></section>${warning}${this.#renderLoadProfilePicker(profiles)}${this.#renderCompatibilityPicker()}</div>`;
    ...
    if (dialog) dialog.state = { accessibleName: "Historical rename warning", message: "Renaming an active or inactive Station can affect how historical workouts are understood. Save this name change?", dismissAction: "dismiss-historical-rename-warning", dismissLabel: "Keep Editing", confirmAction: "save-configurator-station", confirmLabel: "Save Name" };
  }
}
