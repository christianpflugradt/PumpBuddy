...
class PbConfiguratorStationEditorScreenElement extends HTMLElement {
  ...
  #render(): void {
    ...
    const metadata = station && historical ? `<dl class="configurator-metadata"><div><dt>Gym</dt><dd>${escapeHtml(this.#state.gymName ?? this.#state.gymId)}</dd></div><div><dt>Load Profile</dt><dd>${escapeHtml(station.load_profile.name)}</dd></div><div><dt>Status</dt><dd>${station.status === "active" ? "Active" : "Inactive"}</dd></div></dl>` : "";
