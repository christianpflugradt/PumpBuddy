...
describe("pb-configurator-exercise-variant-editor-screen", () => {
  ...
  it("reports semantic field and staged compatibility changes to the shared exit guard", () => {
    ...
    name.value = "Pronated"; name.dispatchEvent(new Event("input", { bubbles: true }));
    ...
    expect(handler.mock.calls.filter((call) => call[0].detail.action === "configurator-draft-state-changed").map((call) => call[0].detail.payload)).toEqual([
      { source: "configurator-exercise-variant-detail", isDirty: true },
      { source: "configurator-exercise-variant-detail", isDirty: false },
      { source: "configurator-exercise-variant-detail", isDirty: true },
      { source: "configurator-exercise-variant-detail", isDirty: false },
    ]);
  });
  ...
  it("emits parent-scoped draft save and delete actions", () => {
    ...
    const save = handler.mock.calls.find((call) => call[0].detail.action === "save-configurator-exercise-variant")![0].detail;
    ...
    const deletion = handler.mock.calls.find((call) => call[0].detail.action === "delete-configurator-exercise-variant")![0].detail;
  ...
  it.each(["active", "inactive"] as const)("requires confirmation before saving a renamed %s variant", (status) => {
    ...
    (el.querySelector('[data-ui-action="save-configurator-exercise-variant"]') as HTMLButtonElement).click();
    expect(handler.mock.calls.some((call) => call[0].detail.action === "save-configurator-exercise-variant")).toBe(false);
    ...
    (el.querySelector('[data-ui-action="save-configurator-exercise-variant"]') as HTMLButtonElement).click();
    (el.querySelector('[data-ui-action="save-configurator-exercise-variant"]') as HTMLButtonElement).click();
    expect(handler.mock.calls.find((call) => call[0].detail.action === "save-configurator-exercise-variant")![0].detail.payload.request).toEqual({ name: "Neutral" });
  });
  it("renders compact gym-grouped Station rows only for station-required variants", () => {
    ...
    expect(handler.mock.calls.find((call) => call[0].detail.action === "open-configurator-exercise-variant-compatible-station")![0].detail).toEqual({ action: "open-configurator-exercise-variant-compatible-station", payload: { gymId: "gym-1", stationId: "station-1" } });
  ...
  it("filters by Station or Gym and persists only the saved staged selection", () => {
    ...
    expect(handler.mock.calls.some((call) => call[0].detail.action === "save-configurator-exercise-variant-compatibilities")).toBe(false);
    (el.querySelector('[data-ui-action="open-configurator-exercise-variant-compatibility-picker"]') as HTMLButtonElement).click();
    ...
    expect(handler.mock.calls.find((call) => call[0].detail.action === "save-configurator-exercise-variant-compatibilities")![0].detail.payload).toEqual({ exerciseId: "exercise-1", variantId: "variant-1", stationIds: ["station-1", "station-2"] });
  });
