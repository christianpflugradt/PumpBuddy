...
test("createApp keeps finish separate from set completion on the last exercise", async () => {
  ...
  assert.match(
    (app as unknown as FakeAppElement).innerHTML,
    /class="step-actions"[\s\S]*class="step-actions-primary"[\s\S]*class="nav-button nav-button-primary"[\s\S]*data-action="next-set"[\s\S]*class="step-actions-secondary"[\s\S]*data-action="previous-exercise"[\s\S]*data-action="next-exercise"/,
  );
