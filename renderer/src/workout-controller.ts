  ...
  app.addEventListener("pb-ui-action", (event: Event) => {
    ...
    switch (action) {
      ...
      case "delete-configurator-exercise-variant": {
        ...
        void (async () => { try { await deleteConfiguratorExerciseVariant(exerciseId, variantId); const current = state.configuratorExerciseDetailScreen; state = { ...state, configuratorExercisesScreen: state.configuratorExercisesScreen ? { ...state.configuratorExercisesScreen, exercises: state.configuratorExercisesScreen.exercises.map((exercise) => exercise.id === exerciseId ? { ...exercise, variant_count: Math.max(0, exercise.variant_count - 1) } : exercise) } : state.configuratorExercisesScreen, configuratorExerciseDetailScreen: current ? { ...current, variants: current.variants.filter((entry) => entry.id !== variantId) } : current, viewState: { screen: "configurator-exercise-detail", exerciseId } }; render(); detail.respond?.({ ok: true }); void loadConfiguratorExerciseDetailScreenData(exerciseId); } catch (error) { detail.respond?.({ ok: false, errorMessage: getRequestErrorMessage(error, "Unable to delete Variant right now.") }); } })();
        return;
      }
