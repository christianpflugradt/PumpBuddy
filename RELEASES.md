...
The GitHub Actions workflow installs the repository toolchain with `npm ci` and runs `npx semantic-release` so plugin resolution does not depend on floating `npx -p` invocations.
