...
- Criteria Met: The renderer image build is now deterministic through `renderer/Dockerfile` using `package-lock.json` plus `npm ci`, `renderer/.dockerignore` excludes host `node_modules` and `dist` from the Docker build context, and the existing `docker-compose.yml` exposure model still keeps only `renderer` public while `backend` and `postgres` remain internal-only.
