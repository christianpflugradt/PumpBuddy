...
- Criteria Met: The retained `plan-item-*.md` files no longer reference missing `open-item` paths, the current execution set uses a consistent stable item-id convention, and [item-template.md](/Users/cpf/Workspace/personal/PumpBuddy/agent/templates/item-template.md) now documents that same convention for future plan files.
- Evidence: The reviewed commit updates every current [plan-item-*.md](/Users/cpf/Workspace/personal/PumpBuddy/agent/execution/plan-item-01.md) companion to `- Stable item id: \`item-XX\`` and adjusts [item-template.md](/Users/cpf/Workspace/personal/PumpBuddy/agent/templates/item-template.md) so plan references are no longer tied to transient execution-state filenames.
