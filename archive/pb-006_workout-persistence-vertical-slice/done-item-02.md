...
- Evidence: [backend/src/main.rs](backend/src/main.rs#L143) registers `GET /api/gyms` and [backend/src/main.rs](backend/src/main.rs#L206) maps repository results into `{ id, name }` JSON; [backend/src/persistence.rs](backend/src/persistence.rs#L176) defines the dedicated gyms query with `ORDER BY created_at ASC, id ASC`; [backend/src/domain.rs](backend/src/domain.rs#L16) defines the `GymSummary` domain shape matching [api-contract.yaml](agent/design/api-contract.yaml#L22).
