...
- Evidence: Backend implements `/api/hello-world` with SQLx query against `hello_world` table and startup seeding (`backend/src/main.rs`), renderer fetches `/api/hello-world` and assigns `messageElement.textContent` from response value (`renderer/src/main.ts`), and compose topology keeps renderer public with backend/postgres internal (`docker-compose.yml`).
