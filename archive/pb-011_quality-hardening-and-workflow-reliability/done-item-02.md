...
- Evidence: [`agent/scripts/check-quality-artifacts.sh`](/Users/cpf/Workspace/personal/PumpBuddy/agent/scripts/check-quality-artifacts.sh#L6) encodes the exact required artifact set and regeneration commands for backend and renderer coverage badges. [`agent/scripts/run-quality.sh`](/Users/cpf/Workspace/personal/PumpBuddy/agent/scripts/run-quality.sh#L43) integrates the snapshot/verify flow around the existing backend and renderer quality entrypoints so stale artifacts are detected during the CI-aligned `check` path.
