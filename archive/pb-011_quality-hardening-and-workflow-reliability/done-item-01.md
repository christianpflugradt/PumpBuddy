...
- Evidence: The modal renderer and dialog actions are implemented in [renderer/src/app.ts](renderer/src/app.ts#L656) and wired into next-exercise, finish, and cancel handlers in [renderer/src/app.ts](renderer/src/app.ts#L1203) and [renderer/src/app.ts](renderer/src/app.ts#L1365). The previous native-confirm copy is preserved for forward navigation and finish, and the cancel message matches the previous implementation. Coverage for these flows is present in [renderer/src/app.test.ts](renderer/src/app.test.ts#L817) and adjacent tests.
